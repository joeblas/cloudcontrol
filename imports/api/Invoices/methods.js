      ...
      .then(Meteor.bindEnvironment((pdfAsBase64) => {
        ...
        recipient.contacts.forEach(({ firstName, lastName, emailAddress }) => {
          const subject = invoice.status === 'sent' ?
            `[CloudControl] ${ownerName} has sent you an invoice for ${invoiceTotal}` :
            `[CloudControl] Payment confirmation for Invoice #${invoice.number}: ${invoice.subject}`;

          sendEmail({
            from: 'CloudControl <demo@cloudcontrol.com>',
            ...
            attachments: [{
              filename: `cloudcontrol_invoice${invoiceId}.pdf`,
              content: pdfAsBase64,
              encoding: 'base64',
            }],
          });
